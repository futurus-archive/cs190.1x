...
def oneHotEncoding(rawFeats, OHEDict, numOHEFeats):
    ...
    for feat in rawFeats:
        data[OHEDict[feat]] = 1.
    return SparseVector(numOHEFeats, data)
...
def evaluateResults(model, data):
    ...
    """
    return float(data.map(lambda x: computeLogLoss(getP(x.features, model.weights, model.intercept), x.label)).reduce(lambda x, y: x+y)/data.count())
...
def parseHashPoint(point, numBuckets):
    ...
    return LabeledPoint(point.split(",")[0], SparseVector(numBuckets, hashFunction(numBuckets, feats, True)))
...
def computeSparsity(data, d, n):
    ...
    """
    return data.map(lambda x: float(len(x.features.indices))/d).reduce(lambda x, y: x+y)/(data.count())
...
logLossTestBaseline = OHETestData.map(lambda x: computeLogLoss(classOneFracTrain, x.label)).reduce(lambda x, y: x+y)/OHETestData.count()
